...
\frame{\frametitle{The principles I}
	\begin{itemize}
...
\frame{\frametitle{The principles II}
\begin{itemize}
  \item The first thing to do is to figure out if we have nails or something
  else.
  \item The second thing to do is to figure out if we are holding a hammer or
  something else.
  \item And then get the right tool for the job, if we aren't lucky to begin
  with.
  \item And to keep in mind that 'good' is often better than 'best'.
\end{itemize}
}
...
\frame{\frametitle{Pixel Difference (PD)}
    ...
    \item The second tool out of the box happened to work much better --  and so
    for my thesis I used Euclidean distance or $l2$-norm to identify
    articulatory onsets.
	\end{itemize}
...
\frame{\frametitle{Pixel Difference (PD): The maths even more simply}
